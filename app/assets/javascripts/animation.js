...
//Convenience wrapper around requestAnimFrame
function animateCallback(options){
  var callback = options.callback;
  var afterCallback = options.after || function(){};
  var beforeCallback = options.before || function(){};

  var transitionFunction = options.transition || Transitions.linear;

  var duration = options.duration || 500; //millis
  ...
  var animloop = function(time){
    if(time < end){
      requestAnimFrame(animloop, bounds);
      var ratioDone = transitionFunction((time - start) / duration);
...
function animateForever(options){
  var callback = options.callback;
  var bounds = options.bounds || null;
...
/*
  *
  * Transition Functions from Scripty 2 penner.js
  *
  */
